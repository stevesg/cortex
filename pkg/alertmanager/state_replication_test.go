...
func TestStateReplication_Settle(t *testing.T) {
	...
	for _, tt := range tc {
		t.Run(tt.name, func(t *testing.T) {
			...
			s := newReplicatedStates("user-1", tt.replicationFactor, replicator, log.NewLogfmtLogger(os.Stdout), reg)
			...
			settleRetryTimeout = 5 * time.Second
			settleRetryInterval = 1 * time.Second
