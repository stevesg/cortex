...
// getReplicasForUser returns the addresses for each _other_ replica assigned to a user.
func (am *MultitenantAlertmanager) getOtherReplicasForUser(userID string) ([]string, error) {
	if am.ring.InstancesCount() <= 0 {
		return nil, fmt.Errorf("InstancesCount <= 0")
	}
	...
	// Don't return our own address in the list.
	selfAddress := am.ringLifecycler.GetInstanceAddr()

	result := make([]string, 0, len(replicationSet.Instances))
...
// ReadFullSateForUser attempts to read the full state from each replica for user. Note that it will try to obtain and return
// state from all replicas, but will consider it a success if state is obtained from only a single replica.
func (am *MultitenantAlertmanager) ReadFullStateForUser(ctx context.Context, userID string) ([]*clusterpb.FullState, error) {
	addrs, err := am.getOtherReplicasForUser(userID)
	if err != nil {
		return nil, err
	}
